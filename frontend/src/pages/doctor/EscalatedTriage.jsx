...
const EscalatedTriage = () => {
  const { data, isLoading, error, refetch } = useFetch(getEscalatedSessions);
  const sessions = data?.data || [];

  return (
    <div className="animate-fade-in">
      <h1 className="page-title mb-6">Escalated Triage Cases</h1>
      <Card>
        <CardHeader title="Cases Assigned to You" />
        ...
        {!isLoading && !error && sessions.length === 0 && (
          <EmptyState
            ...
            description="Triage sessions escalated to you by an admin will appear here."
          />
        )}
        {!isLoading && !error && sessions.length > 0 && (
