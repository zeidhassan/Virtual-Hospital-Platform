...
const AdminOrders = () => {
  ...
  // Status is applied server-side; the free-text search only matches over
  // the current page, same tradeoff as admin/Appointments.jsx.
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <PageHeader title="Pharmacy Orders" />
      ...
      <Card>
        ...
        {!isLoading && !error && filteredOrders.length === 0 && (
          <EmptyState icon={Package} title={searchQuery || statusFilter !== 'all' ? 'No matching orders' : 'No orders'} description={searchQuery || statusFilter !== 'all' ? 'Try adjusting your search or filters.' : 'Pharmacy orders will appear here.'} />
        )}
        {!isLoading && !error && filteredOrders.length > 0 && (
              ...
              return (
                <div key={order.id} className={clsx('p-4 rounded-xl border-l-[5px] bg-surface-subtle hover:bg-surface-warm transition-colors cursor-pointer', statusBorderColor(order.status))} onClick={() => setDetailModal(order)}>
                  <div className="flex items-start gap-4">
                    <div className="w-[46px] h-[46px] rounded-xl bg-amber-100 flex items-center justify-center flex-shrink-0">
                      ...
                      <div className="flex items-center gap-4 text-xs text-text-muted flex-wrap">
                        ...
                        {order.prescription_file_url && (
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              openAuthedFile(`/files/prescriptions/${fileNameFromPath(order.prescription_file_url)}`);
                            }}
                ...
                {(Array.isArray(detailModal.medications) ? detailModal.medications : (detailModal.medications || '').split(',').map((m) => m.trim())).map((med, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex items-center gap-2 p-2 rounded-lg bg-surface-subtle border border-slate-200">
                      <Pill size={16} className="text-amber-600 flex-shrink-0" />
            ...
            {detailModal.prescription_file_url && (
              <div>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-50 border border-amber-200 text-sm text-amber-700 hover:bg-amber-100 transition-colors"
                >
                  <FileText size={16} />
                  View Prescription
                </button>
