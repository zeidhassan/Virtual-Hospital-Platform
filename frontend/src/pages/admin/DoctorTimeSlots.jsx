...
const AdminDoctorTimeSlots = () => {
  ...
  return (
    <div className="animate-fade-in">
      <h1 className="page-title mb-6">Doctor Time Slots</h1>

      <Card>
        <CardHeader title="Manage Availability" />

        {loadingDoctors ? (
          <div className="flex justify-center py-8"><Spinner size="lg" /></div>
        ) : (
          <div className="space-y-5">
            <Select label="Select Doctor" value={selectedDoctor}
              onChange={(e) => { setSelectedDoctor(e.target.value); setSlots([]); }}>
              <option value="">Choose a doctor...</option>
              {doctors.map((d) => (
                <option key={d.id} value={d.id}>{d.doctor_name || d.full_name}</option>
              ))}
            </Select>
            ...
            {selectedDoctor && (
              <div className="flex justify-end">
                <Button size="sm" onClick={() => { setForm(EMPTY_FORM); setAddOpen(true); }}>
                  <Plus size={14} className="mr-1.5" /> Add Slot
                </Button>
              </div>
            )}
            ...
            {!loadingSlots && selectedDoctor && slots.length === 0 && (
              <EmptyState icon={Clock} title="No slots"
                description="This doctor has no time slots configured." />
            )}
            ...
            {!loadingSlots && slots.length > 0 && (
              <div className="space-y-2">
                {slots.map((slot) => (
                  <div key={slot.id}
                    className="flex items-center justify-between gap-4 p-3 rounded-xl bg-surface-subtle">
      ...
      <Modal isOpen={addOpen} onClose={() => setAddOpen(false)}
        ...
        footer={
          <>
            ...
            <Button onClick={handleAdd} isLoading={saving}>Add Slot</Button>
          </>
        }
      >
