...
describe('Support Tickets API (Admin)', () => {
  ...
  test('GET /api/support-tickets - should return all tickets', async () => {
    ...
    expect(pool.query).toHaveBeenCalledWith('SELECT * FROM support_tickets ORDER BY id');
  });

  test('GET /api/support-tickets/:id - found', async () => {
    pool.query.mockResolvedValueOnce({
  ...
  test('GET /api/support-tickets/:id - not found', async () => {
    pool.query.mockResolvedValueOnce({ rows: [] });

    const res = await request(app)
      .get('/api/support-tickets/999')
      .set('Authorization', `Bearer ${adminToken}`);
